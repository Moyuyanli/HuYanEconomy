...
> 当前状态说明：本文是实体层重构的设计方案文档。2.0.0 多模块拆分后，相关实现已主要落在 `economy-data`：实体位于 `economy-data/src/main/kotlin/cn/chahuyun/economy/entity` 与 `entity/v2`，DTO 位于 `model/**`，Converter 位于 `converter/**`，数据代理位于 `data/proxy/**`，仓储位于 `data/repository/**`。下文中的 `src/main/kotlin/cn/chahuyun/economy/` 单模块目录示意保留为设计草案语境，不代表当前物理路径。
...
> 2.0.0 当前物理归属以 `economy-data/src/main/kotlin/cn/chahuyun/economy/` 为根；本节结构图描述的是 data 模块内部逻辑分层。
...
Phase 4 only exposes three data-version commands:

```bash
# Show current module data versions
hye entity version

# Switch all modules without copying data
hye entity switch V1
hye entity switch V2

# Migrate from the currently used data version to the target version, then switch successful modules
hye entity migration V1
hye entity migration V2
```

Module versions are persisted in `PluginConfig.entityDataVersions`. Missing modules use V1 by default, so switching to V1 removes V2 routing from the config instead of deleting data.
