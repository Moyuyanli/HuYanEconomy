...
| `bankruptAt` | Date / Long | NULL | 永久破产时间，未破产为空或 0 |
...
### 5.4.1 PrivateBankMainBankDebt — 主银行应急债务表

V1 表名为 `PrivateBankMainBankDebt`，V2 表名为 `hye_private_bank_main_bank_debt`，均以银行编码唯一保存当前应急债务。

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | Int / Long | PK | 主键 |
| `bankCode` | String | UNIQUE | 私人银行编码 |
| `principal` | Double | | 未还本金 |
| `accruedInterest` | Double | | 已累计但未偿还的单利 |
| `lastAccruedAt` | Date / Long | | 上次计息基准时间 |
| `createdAt` | Date / Long | | 首次形成债务时间 |
| `updatedAt` | Date / Long | | 最近变更时间 |
| `repaidAt` | Date / Long | | 最近结清时间，未结清为空或 0 |
...
1. 更新插件版本
2. 启动 mirai-console
3. 执行 `hye repair` 查看范围，再执行对应的 `v1`、`v2` 或 `V1TOV2` 修复
