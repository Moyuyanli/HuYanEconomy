...
class UserFactorEntityProxy : EntityProxy<UserFactorDto> {
    ...
    private fun findV1ById(id: Long): UserFactorDto? = try {
        HibernateFactory.selectOneById(UserFactor::class.java, id)?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<UserFactorDto> = try {
        v1Converter.toDtoList(HibernateFactory.selectList(UserFactor::class.java))
    } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean = try {
        val entity = HibernateFactory.selectOneById(UserFactor::class.java, id)
    ...
    private fun findV2ByUserId(userId: Long): UserFactorDto? = try {
        HibernateFactory.selectOne(UserFactorEntity::class.java, "userId", userId)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<UserFactorDto> = try {
        v2Converter.toDtoList(HibernateFactory.selectList(UserFactorEntity::class.java))
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: UserFactorDto): UserFactorDto = try {
        ...
        val existing = HibernateFactory.selectOne(UserFactorEntity::class.java, "userId", dto.id)
    ...
    private fun deleteV2ByUserId(userId: Long): Boolean = try {
        val entity = HibernateFactory.selectOne(UserFactorEntity::class.java, "userId", userId)
        if (entity == null) false else {
            HibernateFactory.delete(entity)
            true
        }
