...
class PropsEntityProxy : EntityProxy<PropsDataDto> {
    ...
    private fun findV1ById(id: Long): PropsDataDto? = try {
        HibernateFactory.selectOneById(PropsData::class.java, id)?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<PropsDataDto> = try {
        v1Converter.toDtoList(HibernateFactory.selectList(PropsData::class.java))
    } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean = try {
        val entity = HibernateFactory.selectOneById(PropsData::class.java, id)
    ...
    private fun findV2ById(id: Long): PropsDataDto? = try {
        HibernateFactory.selectOneById(PropsDataEntity::class.java, id)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<PropsDataDto> = try {
        v2Converter.toDtoList(HibernateFactory.selectList(PropsDataEntity::class.java))
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: PropsDataDto): PropsDataDto = try {
        ...
        val existing = if (dto.id != 0L) HibernateFactory.selectOneById(PropsDataEntity::class.java, dto.id) else null
    ...
    private fun deleteV2(id: Long): Boolean = try {
        val entity = HibernateFactory.selectOneById(PropsDataEntity::class.java, id)
        if (entity == null) false else {
            HibernateFactory.delete(entity)
            true
        }
