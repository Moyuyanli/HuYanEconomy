...
class UserEntityProxy : EntityProxy<UserInfoDto> {
    ...
    private fun findV1ById(id: Long): UserInfoDto? {
        return try {
            val entity = HibernateFactory.selectOneById(UserInfo::class.java, id.toString())
    ...
    private fun findV1ByQq(qq: Long): UserInfoDto? {
        return try {
            val entity = HibernateFactory.selectOne(UserInfo::class.java, "qq", qq)
            entity?.let { v1Converter.toDto(it) }
        } catch (e: Exception) {
    ...
    private fun findAllV1(): List<UserInfoDto> {
        return try {
            val entities = HibernateFactory.selectList(UserInfo::class.java)
    ...
    private fun saveV1(dto: UserInfoDto): UserInfoDto {
        return try {
            ...
            val saved = HibernateFactory.merge(entity)
    ...
    private fun findV2ById(id: Long): UserInfoDto? {
        return try {
            val entity = HibernateFactory.selectOneById(UserEntity::class.java, id)
    ...
    private fun findV2ByQq(qq: Long): UserInfoDto? {
        return try {
            val entity = HibernateFactory.selectOne(UserEntity::class.java, "qq", qq)
            entity?.let { withBackpacks(v2Converter.toDto(it)) }
        } catch (e: Exception) {
    ...
    private fun findV2ByUserKey(userKey: String): UserEntity? {
        ...
        return try {
            HibernateFactory.selectOne(UserEntity::class.java, "userKey", userKey)
        } catch (e: Exception) {
    ...
    private fun findAllV2(): List<UserInfoDto> {
        return try {
            val entities = HibernateFactory.selectList(UserEntity::class.java)
    ...
    private fun deleteV2(id: Long): Boolean {
        return try {
            val entity = HibernateFactory.selectOneById(UserEntity::class.java, id)
                ?: HibernateFactory.selectOne(UserEntity::class.java, "qq", id)
            if (entity == null) {
                false
            } else {
                HibernateFactory.delete(entity)
    ...
    private fun findExistingV2Entity(dto: UserInfoDto): UserEntity? {
        return findV2ByUserKey(dto.id)
            ?: runCatching { HibernateFactory.selectOne(UserEntity::class.java, "qq", dto.qq) }
    ...
    private fun findBackpacks(userKey: String): List<UserBackpackDto> {
        ...
        val proxy = EntityProxyRegistry.get<UserBackpackDto>("user_backpack") ?: return emptyList()
        return proxy.findWhere { it.userId == userKey }
    }

    private fun saveBackpacks(backpacks: List<UserBackpackDto>, userKey: String) {
        if (backpacks.isEmpty() || userKey.isEmpty()) return
        val proxy = EntityProxyRegistry.get<UserBackpackDto>("user_backpack") ?: return
        proxy.saveAll(backpacks.map { it.copy(userId = userKey) })
    }
