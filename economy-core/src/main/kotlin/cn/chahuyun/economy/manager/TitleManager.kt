...
object TitleManager {
    ...
    fun init() {
        TitleTemplateManager.registerTitleTemplate(
            TitleTemplateSimpleImpl(
                ...
                true, false,
                "[只是个传说]",
                colorHex(Color(0xff7f50)),
                colorHex(Color(0xff6348))
            ),
            TitleTemplateSimpleImpl(
                ...
                true, true,
                "[大富翁]",
                colorHex(Color(0xff4757)),
                colorHex(Color(0xffa502))
            ),
            TitleTemplateSimpleImpl(
            ...
            TitleTemplateSimpleImpl(
                ...
                true, true,
                "[邓刚]",
                colorHex(Color(0xf02fc2)),
                colorHex(Color(0x6094ea))
            ),
            TitleTemplateSimpleImpl(
                ...
                true, true,
                "[17张牌能秒我?]",
                colorHex(Color(0xFF0000)),
                colorHex(Color(0x730000))
            ),
            TitleTemplateSimpleImpl(
            ...
            TitleTemplateSimpleImpl(
                ...
                true, true,
                "[无敌超级签到大王·神]",
                colorHex(Color(0x622774)),
                colorHex(Color(0xc53364))
            )
        )
    ...
    /**
     * 获取默认称号，优先返回当前启用称号；若无则返回群头衔或默认称号。
     */
    @JvmStatic
    ...
    private fun getInfo(userInfo: UserInfoDto): TitleInfoDto {
        ...
        return if (user is Member) {
            // 避免读取 rankTitle；Overflow 环境下可能触发群活跃信息查询并刷屏 Warning。
    ...
    fun addTitleInfo(userInfo: UserInfoDto, titleTemplateCode: String): Boolean {
        val title = TitleTemplateManager.createTitle(titleTemplateCode, userInfo)
            ?: throw RuntimeException("称号 code 错误，或该称号未在称号模板管理器中注册!")
    ...
    suspend fun checkSignTitle(userInfo: UserInfoDto, subject: Contact) {
        ...
        when {
            signNumber == 15 -> {
                ...
                val builder = MessageChainBuilder()
                builder.append(At(userInfo.qq))
                builder.append("恭喜!你已经连续签到 15 天，获得 15 天称号 [签到狂人]!")
                subject.sendMessage(builder.build())
            }
    ...
    suspend fun checkFishTitle(userInfo: UserInfoDto, subject: Contact) {
        val fishRanking = GameOverviewBridge.topFishRankingWinner() ?: return
        ...
        if (addTitleInfo(userInfo, TitleCode.FISHING)) {
            subject.sendMessage(MessageUtil.formatMessageChain(userInfo.qq, "恭喜你获得钓鱼排行榜榜首!获得 [钓鱼佬] 称号!"))
            if (titleInfo != null) {
                titleProxy.delete(titleInfo.id.toLong())
            }
        }
