...
object RedPackUsecase {
    ...
    suspend fun create(event: GroupMessageEvent) {
        ...
        if (money / number < 0.1) {
            subject.sendMessage(MessageUtil.formatMessageChain(message, "你发的红包太小了，每个红包金额不能低于 0.1！"))
            return
        }
        ...
        if (pack.isRandomAllocation) {
            val doubles = RedPackAllocationService.generateRandomPack(money, number)
        ...
        val prefix = EconomyRuntime.config.prefix
    ...
    private suspend fun grabByPassword(event: GroupMessageEvent, password: String) {
        ...
        if (results.size == 1) {
            ...
            val messageChain = buildMessageChain {
                +At(sender.id)
                +" $password"
                +"\n你抢 $name 抢到 ${MoneyFormatUtil.format(amount)} 枚金币"
            }
            ...
            val messageChain = buildMessageChain {
                ...
                results.forEach { (name, amount) ->
                    +"$name，你抢到了 ${MoneyFormatUtil.format(amount)} 枚金币\n"
                }
            }
            subject.sendMessage(messageChain)
        }
