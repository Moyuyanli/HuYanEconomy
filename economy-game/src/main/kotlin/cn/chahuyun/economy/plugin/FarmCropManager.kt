...
import cn.chahuyun.economy.service.FarmCropStorageService
...
object FarmCropManager {
    ...
    @JvmStatic
    fun init() {
        val crops = FarmCropStorageService.listCrops()
        if (crops.isEmpty()) {
            reloadCrops()
        } else {
            readCrops(crops)
    ...
    private fun reloadCrops() {
        val input = EconomyRuntime.plugin.getResourceAsStream("种子.xlsx")
        if (input == null) {
        ...
        FarmCropStorageService.saveCrops(crops)
        readCrops(crops)
