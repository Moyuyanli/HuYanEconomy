...
import cn.chahuyun.economy.data.repository.RedPackRepository
...
class RedPackEntityProxy : EntityProxy<RedPackDto> {
    ...
    private fun findV1ById(id: Long): RedPackDto? = try {
        RedPackRepository.findById(id.toInt())?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<RedPackDto> = try {
        v1Converter.toDtoList(RedPackRepository.listAll())
    } catch (e: Exception) {
    ...
    private fun saveV1(dto: RedPackDto): RedPackDto = try {
        v1Converter.toDto(RedPackRepository.save(v1Converter.toEntity(dto)))
    } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean = try {
        RedPackRepository.deleteById(id.toInt())
    } catch (e: Exception) {
    ...
    private fun findV2ById(id: Long): RedPackDto? = try {
        RedPackRepository.findV2ById(id)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<RedPackDto> = try {
        v2Converter.toDtoList(RedPackRepository.listAllV2())
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: RedPackDto): RedPackDto = try {
        v2Converter.toDto(RedPackRepository.saveV2(v2Converter.toEntity(dto)))
    } catch (e: Exception) {
    ...
    private fun deleteV2(id: Long): Boolean = try {
        RedPackRepository.deleteV2ById(id)
    } catch (e: Exception) {
