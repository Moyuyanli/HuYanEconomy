...
import cn.chahuyun.economy.data.repository.RaffleBatchRepository
...
class RaffleEntityProxy : EntityProxy<RaffleBatchDto> {
    ...
    private fun findV1ById(id: Long): RaffleBatchDto? = try {
        RaffleBatchRepository.findById(id)?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<RaffleBatchDto> = try {
        v1Converter.toDtoList(RaffleBatchRepository.listAll())
    } catch (e: Exception) {
    ...
    private fun saveV1(dto: RaffleBatchDto): RaffleBatchDto = try {
        v1Converter.toDto(RaffleBatchRepository.save(v1Converter.toEntity(dto)))
    } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean = try {
        RaffleBatchRepository.deleteById(id)
    } catch (e: Exception) {
    ...
    private fun findV2ById(id: Long): RaffleBatchDto? = try {
        RaffleBatchRepository.findV2ById(id)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<RaffleBatchDto> = try {
        v2Converter.toDtoList(RaffleBatchRepository.listAllV2())
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: RaffleBatchDto): RaffleBatchDto = try {
        ...
        val existing = if (dto.id != 0L) RaffleBatchRepository.findV2ById(dto.id) else null
        ...
        v2Converter.toDto(RaffleBatchRepository.saveV2(entity))
    } catch (e: Exception) {
    ...
    private fun deleteV2(id: Long): Boolean = try {
        RaffleBatchRepository.deleteV2ById(id)
    } catch (e: Exception) {
