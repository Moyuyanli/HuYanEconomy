...
import cn.chahuyun.economy.data.repository.RobInfoRepository
...
class RobInfoEntityProxy : EntityProxy<RobInfoDto> {
    ...
    private fun findV1ById(id: Long): RobInfoDto? = try {
        RobInfoRepository.findById(id)?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<RobInfoDto> = try {
        v1Converter.toDtoList(RobInfoRepository.listAll())
    } catch (e: Exception) {
    ...
    private fun saveV1(dto: RobInfoDto): RobInfoDto = try {
        v1Converter.toDto(RobInfoRepository.save(v1Converter.toEntity(dto)))
    } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean = try {
        RobInfoRepository.deleteById(id)
    } catch (e: Exception) {
    ...
    private fun findV2ByUserId(userId: Long): RobInfoDto? = try {
        RobInfoRepository.findV2ByUserId(userId)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<RobInfoDto> = try {
        v2Converter.toDtoList(RobInfoRepository.listAllV2())
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: RobInfoDto): RobInfoDto = try {
        ...
        v2Converter.toDto(RobInfoRepository.saveV2(entity))
    } catch (e: Exception) {
    ...
    private fun deleteV2ByUserId(userId: Long): Boolean = try {
        RobInfoRepository.deleteV2ByUserId(userId)
    } catch (e: Exception) {
