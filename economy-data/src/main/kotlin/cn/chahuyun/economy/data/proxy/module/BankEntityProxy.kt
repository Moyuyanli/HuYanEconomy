...
import cn.chahuyun.economy.data.repository.BankInfoRepository
...
class BankEntityProxy : EntityProxy<BankInfoDto> {
    ...
    private fun findV1ById(id: Long): BankInfoDto? {
        return try {
            val entity = BankInfoRepository.findById(id.toInt())
    ...
    private fun findV1ByCode(code: String): BankInfoDto? {
        return try {
            val entity = BankInfoRepository.findByCode(code)
            entity?.let { v1Converter.toDto(it) }
        } catch (e: Exception) {
    ...
    private fun findAllV1(): List<BankInfoDto> {
        return try {
            v1Converter.toDtoList(BankInfoRepository.listAll())
        } catch (e: Exception) {
    ...
    private fun saveV1(dto: BankInfoDto): BankInfoDto {
        return try {
            v1Converter.toDto(BankInfoRepository.save(v1Converter.toEntity(dto)))
        } catch (e: Exception) {
    ...
    private fun deleteV1(id: Long): Boolean {
        return try {
            BankInfoRepository.deleteById(id.toInt())
        } catch (e: Exception) {
    ...
    private fun findV2ById(id: Long): BankInfoDto? {
        return try {
            val entity = BankInfoRepository.findV2ById(id)
    ...
    private fun findV2ByCode(code: String): BankInfoDto? {
        return try {
            val entity = BankInfoRepository.findV2ByCode(code)
            entity?.let { v2Converter.toDto(it) }
        } catch (e: Exception) {
    ...
    private fun findAllV2(): List<BankInfoDto> {
        return try {
            v2Converter.toDtoList(BankInfoRepository.listAllV2())
        } catch (e: Exception) {
    ...
    private fun saveV2(dto: BankInfoDto): BankInfoDto {
        return try {
            ...
            v2Converter.toDto(BankInfoRepository.saveV2(entity))
        } catch (e: Exception) {
    ...
    private fun deleteV2(id: Long): Boolean {
        return try {
            BankInfoRepository.deleteV2ById(id)
        } catch (e: Exception) {
    ...
    private fun findExistingV2Entity(dto: BankInfoDto): BankEntity? {
        if (dto.id != 0) {
            return runCatching { BankInfoRepository.findV2ById(dto.id.toLong()) }
                .onFailure { Log.error("BankEntityProxy query V2 bank failed: id=${dto.id}", it) }
                .getOrNull()
        }

        if (dto.code.isEmpty()) return null
        return runCatching { BankInfoRepository.findV2ByCode(dto.code) }
