...
import cn.chahuyun.economy.data.repository.UserStatusRepository
...
class UserStatusEntityProxy : EntityProxy<UserStatusDto> {
    ...
    private fun findV1ById(id: Long): UserStatusDto? = try {
        UserStatusRepository.findById(id)?.let { v1Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV1(): List<UserStatusDto> = try {
        v1Converter.toDtoList(UserStatusRepository.listAll())
    } catch (e: Exception) {
    ...
    private fun saveV1(dto: UserStatusDto): UserStatusDto = try {
        v1Converter.toDto(UserStatusRepository.save(v1Converter.toEntity(dto)))
    } catch (e: Exception) {
    ...
    private fun findV2ByUserId(userId: Long): UserStatusDto? = try {
        UserStatusRepository.findV2ByUserId(userId)?.let { v2Converter.toDto(it) }
    } catch (e: Exception) {
    ...
    private fun findAllV2(): List<UserStatusDto> = try {
        v2Converter.toDtoList(UserStatusRepository.listAllV2())
    } catch (e: Exception) {
    ...
    private fun saveV2(dto: UserStatusDto): UserStatusDto = try {
        ...
        v2Converter.toDto(UserStatusRepository.saveV2(entity))
    } catch (e: Exception) {
    ...
    private fun deleteV2ByUserId(userId: Long): Boolean = try {
        UserStatusRepository.deleteV2ByUserId(userId)
    } catch (e: Exception) {
